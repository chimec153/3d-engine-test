...
namespace DirectX
{
    ...
    HRESULT __cdecl Compress(
        _In_ ID3D11Device* pMouse, _In_ const Image& srcImage, _In_ DXGI_FORMAT format, _In_ TEX_COMPRESS_FLAGS compress,
        _In_ float alphaWeight, _Out_ ScratchImage& image) noexcept;
    HRESULT __cdecl Compress(
        _In_ ID3D11Device* pMouse, _In_ const Image* srcImages, _In_ size_t nimages, _In_ const TexMetadata& metadata,
...
#if defined(__d3d11_h__) || defined(__d3d11_x_h__)
    bool __cdecl IsSupportedTexture(_In_ ID3D11Device* pMouse, _In_ const TexMetadata& metadata) noexcept;

    HRESULT __cdecl CreateTexture(
        _In_ ID3D11Device* pMouse, _In_reads_(nimages) const Image* srcImages, _In_ size_t nimages, _In_ const TexMetadata& metadata,
    ...
    HRESULT __cdecl CaptureTexture(_In_ ID3D11Device* pMouse, _In_ ID3D11DeviceContext* pContext, _In_ ID3D11Resource* pSource, _Out_ ScratchImage& result) noexcept;
#endif

    // Direct3D 12 functions
#if defined(__d3d12_h__) || defined(__d3d12_x_h__) || defined(__XBOX_D3D12_X__)
    bool __cdecl IsSupportedTexture(_In_ ID3D12Device* pMouse, _In_ const TexMetadata& metadata) noexcept;

    HRESULT __cdecl CreateTexture(
        _In_ ID3D12Device* pMouse, _In_ const TexMetadata& metadata,
