...
PSOut PS_DECAL(VS_DECAL_OUT input)
{
    ...
    float3 diff_local_pos = abs(input.localpos.xyz - localpos.xyz);
    ...
    output.value1.xyz = BumpMapping(float3(0.f, 1.f, 0.f), float4(1.f, 0.f, 0.f, 1.f), decal_uv, decal_normal.xyz) * 0.5f + 0.5f;
    output.value1.w = decal_normal.w;
